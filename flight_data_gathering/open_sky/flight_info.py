...
class HistoricalDataManager(object):
    def __init__(self,
                 file_batch_unit: str = 'daily',
                 time_interval: datetime.timedelta = datetime.timedelta(minutes=1),
                 on_ground: bool = False,
                 min_ft: int = 33000):
        """

        Args:
            file_batch_unit (str): select from 'daily', 'monthly'
            time_interval (datetime.timedelta): time interval
            on_ground (bool): If True, the airplane is on the ground
            min_ft (int): minimum feet for data
        """
    ...
    def get_data_single_date_test(self,
                                  start_datetime, stop_datetime,
                                  # calc_interval_datetime: datetime.timedelta = datetime.timedelta(hours=1),
                                  # tqdm_count: bool = True,
                                  **kwargs_impala_history
                                  ):

        start_datetime_str = start_datetime.strftime('%Y-%m-%d %H:%M')
        stop_datetime_str = stop_datetime.strftime('%Y-%m-%d %H:%M')
        flight = opensky.history(
            start=start_datetime_str,
            stop=stop_datetime_str,
            cached=False,
            **kwargs_impala_history
        )
        # todo: add filter of ground, etc
        df_out = flight.data
        return df_out

    # def get_data_single_date(self,
    #                          target_date: datetime.date,
    #                          callsign: Optional[str, List[str]] = None,
    #                          icao24: Optional[str, List[str]] = None,
    ...
    #
    #     Args:
    #         target_date (datetime.date): target datetime
    #         callsign (): (wildcards accepted, _ for any character, % for any sequence of characters)
    #         icao24 (): identifying i the transponder code of the aircraft
    #         departure_airport ():
    #         arrival_airport ():
    ...
    #             df_out, path_dest = self.get_data_single_date(target_date=target_date,
    #                                                           callsign=callsign,
    #                                                           icao24=icao24,
    #                                                           departure_airport=departure_airport,
    #                                                           arrival_airport=arrival_airport,
...
def test_2():
    ...
    historical_locations_data = HistoricalDataManager(file_batch_unit=file_batch_unit,
                                                      time_interval=time_interval
                                                      )
    df = historical_locations_data.get_data_single_date(target_date=target_date,
                                                        callsign=callsign,
                                                        icao24=icao24,
